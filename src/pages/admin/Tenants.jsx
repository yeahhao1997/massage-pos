      ...
      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
        {rows.map((tn) => (
          <div key={tn.id} className="card">
            ...
            <button className="btn-ghost mt-3 w-full text-xs" onClick={() => toggleStatus(tn)}>
...
function NewTenantModal({ open, onClose, onDone }) {
  const { t } = useT();
  const toast = useToast();
  const [f, setF] = useState({ name: '', city: '', address: '', phone: '', intro: '', owner_username: '', owner_password: '' });
  ...
  async function submit() {
    try {
      ...
      toast.ok(t('门店已入驻'));
      setF({ name: '', city: '', address: '', phone: '', intro: '', owner_username: '', owner_password: '' });
      onDone();
    } catch (e) { toast.err(e.message); }
  }
